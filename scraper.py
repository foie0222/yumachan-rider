...
class Scraper:
    ...
    def get_next_url(self):
        ...
        return str(soup.find("span", class_="pager-next").find("a").get("href"))
    

def isLocal(entry_inner):
    return entry_inner.header.find("a", class_="entry-category-link category-地方競馬") is not None  # 地方競馬ならTrue


def get_date(entry_inner):
    return entry_inner.find("a", class_="entry-title-link bookmark").string[:8] # レース日付を取得


def get_url(entry_inner):
    return entry_inner.find("a", class_="entry-title-link bookmark").get("href") # レースのURLを取得