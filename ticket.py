...
def make_ticket(entry, realtime_odds):
    ...
    for index, horse in enumerate(entry.horse_list[:4]):
        ...
        if horse.probability * odds.tanodds > 90 or horse.sign == 'axis' or index <= 1:  # 購入金額が100円以上もしくは軸の表記がある馬、最も高確率の馬は軸馬リストに追加
            axis_list.append(horse)

        # if odds.tanodds < 3 or bet == 0:  # 単勝オッズが3倍以下なら購入見送り
        #     continue  
    ...
    for horse in entry.horse_list:
        ...
        if horse.probability * odds.tanodds >= 130 or horse.sign == 'braid':  # 購入金額が100円以上か紐の表記がある馬は紐馬リストに追加
            braid_list.append(horse)

        if bet == 0 or is_in_list(horse.umano, axis_list):  # 軸馬なら購入見送り
            continue
        ticket_tan = Ticket(entry.opdt, entry.rcourcecd, entry.rno, 'FUKUSYO', 'NORMAL', '', horse.umano, str(bet))
    ...
    for braid in braid_list:
        for axis in axis_list:
            ...
            if odds.wideodds * braid.probability * axis.probability * 4 < 100:
                continue
